...
public class SpinnerTitleAdapter extends ArrayAdapter<String> {
    ...
    public SpinnerTitleAdapter(Context activitySpinner, int textViewResourceId, List objects, Resources resLocal) {
    ...
    public SpinnerTitleAdapter(FragmentActivity activitySpinner, int textViewResourceId, List objects, Resources resLocal) {
        super(activitySpinner, textViewResourceId, objects);
    ...
    public View getCustomView(int position, View convertView, ViewGroup parent) {
        ...
        model = null;
        model = (TitleItem) data.get(position);

        TextView textView = (TextView) row.findViewById(R.id.row_item);

        if(position == 0) {
            textView.setText("กรุณาเลือกคำนำหน้าชื่อ");
